...
- ✓ Orchestrator skill v1.1 update — `malware-analysis-orchestrator` SKILL.md rewritten for IDA-first backend priority (D-01), 22-gateway-native + 32-extension tool surface (D-02), dual-mode operation (gateway vs scripts; D-07/D-09), 7-workflow decision tree routing to `references/workflows/W-1..W-7-*.md` (D-04, table-form per D-05), dynamic-mode skip behavior (D-18); `references/workflow.md` + `references/deep-analysis-checklist.md` swept with priority fix + W-N cross-references; `scripts/update_state.py` extended with `mode`/`dynamic_mode_enabled`/`dynamic_capabilities` keys (D-19); `scripts/init_status_tree.sh::populate_dynamic_caps` mode-aware probe (curl-against-MCP for gateway, repo-root probe for scripts) with HI-01/02/03 correctness fixes (set +e rc capture, command -v qemu arch loop, MCP Streamable HTTP Accept header + awk SSE strip); skill-side `scripts/probe_dynamic_tools.sh` wrapper execs the repo-root Phase 11 probe; `mcp-gateway/tests/test_skill_md_dual_mode.py` 52-test regression suite GREEN with SKILL.md sha256 snapshot baseline (`mcp-gateway/tests/snapshots/SKILL.md.sha256`) — Validated in Phase 12: orchestrator-skill-update (SKILL-01, SKILL-02, SKILL-03, SKILL-04)
- ✓ Atomic concurrency caps + r2 sandbox boundary — `asyncio.BoundedSemaphore` replaces TOCTOU `count >= max` on `SessionRegistry` (r2 + gdb combined cap) and `BackgroundJobRegistry` (single release sink in `_mark_terminal` across 7 terminal-state paths); r2 sessions latch `e cfg.sandbox=true` via post-spawn stdin batch (Phase 13 hot-fix `d696a72` after argv-based approach was found to activate sandbox AFTER binary autoload); `_DANGEROUS_R2_CMD_RE` reframed as defence-in-depth marker (`DO NOT EXTEND` + `PHASE 13 SECURITY BOUNDARY DELINEATION`); env-gated `open_r2_session_unsafe` opt-in (`MCP_GATEWAY_R2_UNSAFE_ALLOWED=1`) with WARN-level audit logging — Validated in Phase 13: harden-concurrency-caps-and-r2-sandboxing (HARDEN-01..07, SESS-CAP-01, JOBS-CAP-01)
- ✓ v1.1 milestone gap closure — Test-order regressions fixed (`r2_sessions` module-attribute catch, `sessions/__init__` package re-bind, ACL test container-only skipif); REQUIREMENTS.md sync (14 checkbox flips + 14 traceability rows); ROADMAP/STATE/VALIDATION drift cleared (4 phase `nyquist_compliant: true` flips); 15 live UAT items captured in rebuilt container `kali-re-tools:0ac0f3e3ebbf` across phases 7/8/10/11/13; `/gsd-audit-milestone v1.1` re-run returned `status: passed`, `gaps: []` — Validated in Phase 14: close-v1.1-gaps

### Active

(None yet — v1.2 milestone not yet scoped. Run `/gsd-new-milestone` to define active requirements.)
...
**Shipped:** v1.1 Remote RE Tool Expansion (2026-05-21) — see [.planning/MILESTONES.md](MILESTONES.md). Tool surface 22 → 54 baseline / 61 with dynamic mode. Constrained `run_shell` + 11 typed static wrappers + session-scoped r2/gdb + background job system + extraction tier (binwalk3 + unblob + UPX) + env-gated dynamic mode (`./run_docker.sh --dynamic`) + orchestrator skill rewrite (IDA-first) + atomic concurrency caps + r2 `cfg.sandbox=true` boundary. 61/61 requirements satisfied; audit `status: passed`.

**Carryover Tech Debt to v1.2** (logged in `.planning/phases/14-close-v1.1-gaps/deferred-items.md`): 47 REQUIREMENTS.md traceability rows still show `TBD | Pending` in `Plan | Verified` columns; 4 phase VERIFICATION.md frontmatters labelled `human_needed` despite in-document closure; `test_r2_sessions.py` conftest monkey-patch bypass (in-container); `test_skill_md_dual_mode.py` `StopIteration` (host-only by intent); MCP `r2_cmd` 30 s session-pipe timeout on freshly-opened sessions; `test_mastra_starter.py` tsx 4.21 + Node v25 wrapper-script skew (pin tsx ≥4.22). None affect v1.1 invariants or the security boundary.

## Next Milestone

Not yet scoped. Run `/gsd-new-milestone` to define v1.2 goals, requirements, and roadmap.

<details>
<summary>v1.1 Remote RE Tool Expansion (Shipped 2026-05-21) — original milestone goals</summary>
...
| IDA-first backend priority in skill docs (D-01) | Fixes drift from skill's pre-v1.1 BN-first language; matches gateway's `PinnedBackend` IDA > BN > Ghidra fallback chain | ✓ Done — Phase 12 |
| Dual-mode operation section + Workflow Decision Tree routing to W-1..W-7 (D-04, D-07) | Skill must read identically when an agent runs inside-container (scripts/) vs as a remote gateway client; W-N table form gives both paths in one place | ✓ Done — Phase 12 |
| Dynamic-mode skip behavior in skill (D-18) | Dynamic tools (`run_strace`/`run_ltrace`/`run_qemu_user`/gdb) are env-gated default-off; skill must not assume their presence — probes capabilities at case-init via `populate_dynamic_caps` | ✓ Done — Phase 12 |
| Skill-side `scripts/probe_dynamic_tools.sh` is a thin wrapper exec'ing the repo-root Phase 11 probe | Keeps W-7 reference resolvable by the basename-based test resolver without duplicating probe logic | ✓ Done — Phase 12 |
| `asyncio.BoundedSemaphore` for SessionRegistry + BackgroundJobRegistry caps (atomic probe-and-acquire under `_lock`) | TOCTOU `count >= max` allowed N+1 callers to slip past cap=N under concurrency; semaphore + single release sink in `_mark_terminal` makes cap enforcement reload-safe and double-release-proof | ✓ Done — Phase 13 |
| r2 `cfg.sandbox=true` latched via post-spawn stdin batch (not argv) | Phase 13 hot-fix `d696a72`: argv `-e cfg.sandbox=true` was eval'd AFTER binary autoload, leaving a window. stdin-time latch occurs BEFORE any user `r2_cmd` reaches r2's parser | ✓ Done — Phase 13 hot-fix |
| Security boundary lives on `cfg.sandbox`, not the `_DANGEROUS_R2_CMD_RE` regex | Regex was a defence-in-depth UX layer; chasing an r2-command parser-arms-race is unwinnable. `cfg.sandbox=true` enforced at argv-eval time blocks `r_sandbox_system` + upper-dir open at r2 core layer | ✓ Done — Phase 13 |
| Env-gated `open_r2_session_unsafe` (`MCP_GATEWAY_R2_UNSAFE_ALLOWED=1`) as a SEPARATE tool, not a kwarg | Audit visibility from outside the container via `tools/list`; WARN-level log on every open carries session_id + sample_sha256[:8] + case_dir for grep | ✓ Done — Phase 13 |
| Phase 14 as a dedicated meta-audit / gap-closure phase (not bookkeeping inside other phases) | Surgical scope (≤50-line diffs per plan, plan-by-plan SUMMARY) made the gap-closure auditable; let `/gsd-audit-milestone v1.1` re-run serve as the success oracle (`status: passed`) | ✓ Done — Phase 14 |
...
---
*Last updated: 2026-05-21 after v1.1 milestone shipped. All 10 v1.1 phases (5-14) complete; 61/61 requirements validated; audit `status: passed`; tool surface 22 → 54 baseline / 61 with dynamic mode.*
