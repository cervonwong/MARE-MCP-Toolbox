...
**Installation pattern:** Same as Binary Ninja -- user provides IDA Pro installation at build time via Docker build context, `ida-mcp` installed via pip/uv at build time, `configure-agent-mcp.sh` detects and registers at runtime.
...
pip install ida-mcp==2.1.0

# MCP proxy for remote access
pip install mcp-proxy

# Or with uv (faster)
uv pip install ida-mcp mcp-proxy
```
