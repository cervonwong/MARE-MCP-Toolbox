...
- **D-02:** Atomic tier = one tool per 13-artifact file. **Gateway-native** surface (atomic + workflow + case + `get_active_backend`) = ~19 tools. **Backend disassembler tools** pass through with their native names and schemas (count varies by backend — Ghidra ≈ 50+, IDA ≈ 30–40, BN varies). GW-02's "15–25 tools" cap applies to the gateway-native surface only; backend pass-through is additive and explicitly out of that budget. This decision was ratified after prototyping showed that forcing a unified disasm surface would require arg-shape normalization (e.g., Ghidra's `session_id` + `function_start` vs IDA's pre-bound program model) that loses backend power without simplifying clients — see `02-DISCUSSION-LOG.md` "Option 2 pivot".
- **D-03:** Tool naming — **verb-first** style for GATEWAY-NATIVE tools: `run_triage`, `collect_strings`, `scan_yara`, `rank_signals`, `build_hypothesis`, `list_cases`, `get_artifact`, `set_active_case`, `get_active_backend`, etc. Avoid dotted namespaces and underscored-by-domain names for these. **Backend pass-through tools keep their native names unchanged** (e.g., Ghidra: `decomp.function`, `function.list`, `program.open`; IDA: `decompile`, `list_funcs`). The gateway does NOT rename or wrap backend tools.
- **D-04:** Case/session state is exposed as gateway-native tools: `list_cases`, `get_artifact(case_id, artifact_name)`, `set_active_case`, `get_active_case`. Phase 2 tracks the active case as **process-global state** (single-session model — matches the discretion note below allowing "single-session or simple serialization"). Per-MCP-session tracking is deferred to v2 (GW-V2-03). In Phase 4 these artifacts are additionally promoted to MCP Resources (CLI-04). Note: the active case governs gateway pipeline tools only (where artifacts get written); the backend's own session/program lifecycle (e.g., Ghidra `program.open` → `session_id`) is a **client concern** under the pass-through model — clients call the backend's native open/session tools and thread the resulting IDs themselves.
- **D-05:** No raw CLI passthrough (already ruled out in REQUIREMENTS.md Out of Scope). Gateway-native tools are curated wrappers with documented JSON schemas. Backend pass-through tools inherit the backend's own MCP schemas — those schemas are the backend's own contract and are not "raw CLI" in the REQUIREMENTS.md sense.
...
- **D-07:** Gateway acts as a **transparent pass-through** for the pinned backend's MCP tools. At gateway startup (inside the Starlette lifespan) it calls `tools/list` on the backend's ClientSession, and for each backend tool it registers a forwarding handler on the gateway's FastMCP instance under the tool's **native name** with the backend's **native input schema**. Client calls to those tools are forwarded verbatim (name + args) through `PinnedBackend.call_tool(name, args)`. Session/program lifecycle (e.g., Ghidra `program.open` → `session_id` threading) is the client's responsibility; the gateway does NOT translate args, rename tools, or manage session IDs. A new gateway-native tool `get_active_backend() → "ida" | "bn" | "ghidra"` lets clients (and the orchestrator skill) learn which native surface is currently exposed and branch accordingly.
